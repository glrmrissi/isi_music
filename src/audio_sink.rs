...
impl BandAnalyzer {
    pub fn new(bands: Arc<Mutex<Vec<f32>>>) -> Self {
    ...
    fn compute_bands(&mut self) {
        ...
        if let Ok(mut bands) = self.bands.lock() {
            for (cur, &next) in bands.iter_mut().zip(self.new_bands_buf.iter()) {
                if next > *cur { *cur = next; } else { *cur *= 0.88; }
            }
        }
    }
}
...
impl AnalyzerSink {
    pub fn new(inner: Box<dyn Sink>, bands: Arc<Mutex<Vec<f32>>>) -> Self {
        Self { inner, analyzer: BandAnalyzer::new(bands) }
    }
}
...
impl Sink for AnalyzerSink {
    ...
    fn stop(&mut self) -> SinkResult<()> {
        if let Ok(mut bands) = self.analyzer.bands.lock() {
    ...
    fn write(&mut self, packet: AudioPacket, converter: &mut Converter) -> SinkResult<()> {
        if let AudioPacket::Samples(ref samples) = packet {
            self.analyzer.push_f64_stereo(samples);
        }
    ...
    S: rodio::Source<Item = f32>,
{
    pub fn new(inner: S, bands: Arc<Mutex<Vec<f32>>>) -> Self {
        ...
        Self {
            inner,
            analyzer: BandAnalyzer::new(bands),
            channels,
            channel_pos: 0,
            mix_acc: 0.0,
        }
